...
def espera_api_estar_ativa(url=ROOT_URL, timeout=30, poll_interval=5):
...
# --- Fixtures para Criação e Limpeza de Recursos ---
@pytest.fixture
def aluno_data():
    return {
        "nome": "Carlos",
...
def criar_professor(professor_data):
    ...
    yield professor_id
    requests.delete(f"{BASE_URL}/professores/{professor_id}")
    
@pytest.fixture(scope="function")
def criar_turma(turma_data):
...
def criar_aluno(aluno_data):
    ...
    yield aluno_id
    requests.delete(f"{BASE_URL}/alunos/{aluno_id}")
...
def test_post_aluno(aluno_data):
    ...
    try:
        ...
        # Limpeza imediata para não depender de outros testes
        requests.delete(f"{BASE_URL}/alunos/{response_json['id']}")
    except requests.exceptions.RequestException as e:
        pytest.fail(f"Erro ao criar aluno: {e}")
        print(f"TESTE: test_post_aluno FALHOU COM ERRO: {e}")

def test_get_aluno(criar_aluno, aluno_data):
    aluno_id = criar_aluno
    print(f"\nEXECUTANDO TESTE: test_get_aluno COM ID: {aluno_id}")
    try:
        ...
        pytest.fail(f"Erro ao obter aluno: {e}")
        print(f"TESTE: test_get_aluno FALHOU COM ERRO: {e}")

def test_put_aluno(criar_aluno):
    aluno_id = criar_aluno
    aluno_update = {
        "nome": "Carlos Eduardo",
    ...
    print(f"\nEXECUTANDO TESTE: test_put_aluno COM ID: {aluno_id}")
    try:
        response = requests.put(f"{BASE_URL}/alunos/{aluno_id}", json=aluno_update)
        response.raise_for_status()
        verificar_status_ok(response)
        response_json = response.json()
        assert response_json["id"] == aluno_id
        ...
        pytest.fail(f"Erro ao atualizar aluno: {e}")
        print(f"TESTE: test_put_aluno FALHOU COM ERRO: {e}")

def test_delete_aluno(criar_aluno):
    aluno_id = criar_aluno
    print(f"\nEXECUTANDO TESTE: test_delete_aluno COM ID: {aluno_id}")
    try:
        ...
        pytest.fail(f"Erro ao deletar aluno: {e}")
...
def test_post_professor(professor_data):
    ...
    try:
        ...
        print(f"TESTE: test_post_professor CONCLUÍDO (Professor ID: {response_json['id']} criado)")
        # Limpeza imediata
        requests.delete(f"{BASE_URL}/professores/{response_json['id']}")
    except requests.exceptions.RequestException as e:
        pytest.fail(f"Erro ao criar professor: {e}")
        print(f"TESTE: test_post_professor FALHOU AO CRIAR COM ERRO: {e}")

def test_get_professor(criar_professor, professor_data):
    professor_id = criar_professor
    print(f"\nEXECUTANDO TESTE: test_get_professor COM ID: {professor_id}")
    try:
        ...
        pytest.fail(f"Erro ao obter professor: {e}")
        print(f"TESTE: test_get_professor FALHOU COM ERRO: {e}")

def test_put_professor(criar_professor):
    professor_id = criar_professor
    professor_update = {
        "nome": "Ana Paula Lima Atualizada",
        "disciplina": "Física Avançada",
    }
    print(f"\nEXECUTANDO TESTE: test_put_professor COM ID: {professor_id}")
    try:
        response = requests.put(f"{BASE_URL}/professores/{professor_id}", json=professor_update)
        response.raise_for_status()
        verificar_status_ok(response)
        response_json = response.json()
        assert response_json["id"] == professor_id
        ...
        pytest.fail(f"Erro ao atualizar professor: {e}")
        print(f"TESTE: test_put_professor FALHOU COM ERRO: {e}")

def test_delete_professor(criar_professor):
    professor_id = criar_professor
    print(f"\nEXECUTANDO TESTE: test_delete_professor COM ID: {professor_id}")
    try:
        ...
        pytest.fail(f"Erro ao deletar professor: {e}")
...
def test_post_turma(turma_data):
    ...
    try:
        ...
        # Limpeza imediata
        requests.delete(f"{BASE_URL}/turmas/{response_json['id']}")
    except requests.exceptions.RequestException as e:
        pytest.fail(f"Erro ao criar turma: {e}")
        print(f"TESTE: test_post_turma FALHOU COM ERRO: {e}")

def test_get_turma(criar_turma, turma_data):
    turma_id = criar_turma
    print(f"\nEXECUTANDO TESTE: test_get_turma COM ID: {turma_id}")
    try:
        ...
        pytest.fail(f"Erro ao obter turma: {e}")
        print(f"TESTE: test_get_turma FALHOU COM ERRO: {e}")

def test_put_turma(criar_turma):
    turma_id = criar_turma
    turma_update = {
        "nome": "Turma F",
        "turno": "Manhã",
    ...
    print(f"\nEXECUTANDO TESTE: test_put_turma COM ID: {turma_id}")
    try:
        response = requests.put(f"{BASE_URL}/turmas/{turma_id}", json=turma_update)
        response.raise_for_status()
        verificar_status_ok(response)
        response_json = response.json()
        assert response_json["id"] == turma_id
        ...
        pytest.fail(f"Erro ao atualizar turma: {e}")
        print(f"TESTE: test_put_turma FALHOU COM ERRO: {e}")

def test_delete_turma(criar_turma):
    turma_id = criar_turma
    print(f"\nEXECUTANDO TESTE: test_delete_turma COM ID: {turma_id}")
    try:
        ...
        pytest.fail(f"Erro ao deletar turma: {e}")
